...
| `04_mab_glorys_tides_reservoirs.jl` | `02`'s GLORYS boundaries, but now WITH the M2..Mm tide added on top (additively — see the script's header for why `tidal_boundary_conditions` isn't used directly), `TracerReservoir` on T/S instead of `NormalRadiation`, and `FilteredTimeInterval` de-tided daily/pentad output alongside raw hourly. This one DOES need three of the five bundled branches (`TracerReservoir`, tides, `FilteredTimeInterval`) plus `TPXO10Atlas`, so unlike `02` it can only run here. First validated 2026-09-20 on a 14-day run — see `NumericalEarth/CLAUDE.md`'s session log for the results and the known east-edge spike still worth another look. Also supports checkpoint/restart (`MAB_CHECKPOINT_EVERY`, `MAB_PICKUP`) — see the script's own comment above the `Checkpointer` for the offset needed to keep the de-tided output continuous across a restart (validated on a 30-day run, `mab_1month_continuous`, 2026-09-21). Saves full-depth u/v/w/T/S daily (`:volume_daily`, same de-tided cadence as `:surface_daily`) in addition to the top-level-only fields, for transects and anything else that needs more than the surface. |
| `plot_temperature_transects.jl` | Longitude-depth (x-z) temperature transects at three fixed latitudes (south/middle/north of the domain), from a run's final checkpoint (`MAB_TAG`, optionally `MAB_CHECKPOINT` for a specific one) rather than a JLD2Writer file — the checkpoint has the complete model state, which is what a full-depth snapshot needs. Saves both a full-depth PNG and a top-500m zoom. Older runs without `:volume_daily` (see `04`, above) have no other way to get a full-depth snapshot; newer runs could read `{TAG}_volume_daily.jld2` directly instead. |
...
`04`'s `FilteredTimeInterval` daily output (`{TAG}_eta_daily.jld2`/
